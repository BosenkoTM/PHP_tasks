...
Q30. И self`, и this являются ключевыми словами, которые можно использовать для ссылки на переменные-члены класса. Разница состоит в том, что $this->member следует использовать для ____ членов, а self::$member следует использовать для _____членов.
a)	частных, публичных
...
c)	return
d)	continue
Q36. Оператор php НЕ определяется спецсимволом !. Учитывая фрагмент ниже, есть ли выход из цикла и какой его результат?
<?php
$num = 21;
...
Q51. Что такое геттеры и сеттеры?
...
Q59. PHP "variable variable" принимает значение переменной и обрабатывает его как имя переменной. Например, если $var это переменная, то $$var это переменная переменной, имя которой является значением $var. Какой скрипт создает вывод ниже, используя переменную переменных?
Cat
Dog
Dog
a)
...
Q78. Вы хотите использовать подстановочные знаки при поиске записей в базе данных MySQL/MariaDB с использованием подготовленного оператора PDO. Какой код следует использовать?
...
Q82. Какой цикл выводит все числа от 1 до 10 включительно?
...
Вопрос 97. Учитывая представленный ниже ассоциативный массив, какой PHP-код определяет элемент массива «яблоко»?
$array = array(
'fruit1' => 'apple',
'fruit2' => 'orange',
'fruit3' => 'grape',
...
Вопрос 106. Какая структура управления PHP используется внутри цикла, чтобы пропустить оставшуюся часть кода текущего цикла и вернуться к началу цикла для следующей итерации?
a)	return
b)	else
c)	break
d)	continue
...
Вопрос 124. Какую функцию использовать, чтобы проверить, является ли переменная массивом?
a)	is_array()
b)	array_check()
c)	typeof()
d)	is_array()
...
Вопрос 128. Какую функцию использовать для получения длины строки в PHP?
count()
 length()
 strlen()
 size()
